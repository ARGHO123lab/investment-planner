...
def init_db():
    ...
    cursor = conn.cursor()
    cursor.execute('''CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, mobile TEXT NOT NULL, country TEXT NOT NULL, target_amount REAL DEFAULT 0, target_years INTEGER DEFAULT 0)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS reports (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, income REAL NOT NULL, expense REAL NOT NULL, savings REAL NOT NULL, risk TEXT NOT NULL, created_at TEXT NOT NULL, FOREIGN KEY (user_id) REFERENCES users (id))''')
    columns_to_add = [("sip_calc_monthly", "REAL DEFAULT 0"), ("sip_calc_years", "INTEGER DEFAULT 0"), ("sip_calc_fv", "REAL DEFAULT 0"), ("future_target_amount", "REAL DEFAULT 0"), ("future_target_years", "INTEGER DEFAULT 0"), ("future_req_monthly", "REAL DEFAULT 0")]
    for col_name, col_type in columns_to_add:
        try: cursor.execute(f"ALTER TABLE reports ADD COLUMN {col_name} {col_type}")
...
def dashboard():
    ...
    report_data = {"country": user['country'], "currency": currency, "income": report['income'], "expense": report['expense'], "savings": savings, "risk": risk.capitalize(), "sip": savings * rules['sip'], "large_cap": savings * rules['large_cap'], "mid_cap": savings * rules['mid_cap'], "small_cap": savings * rules['small_cap'], "emergency_fund": savings * rules['emergency'], "advice": ADVISOR_INSIGHTS.get(risk, []), "sip_calc_monthly": report['sip_calc_monthly'], "sip_calc_years": report['sip_calc_years'], "sip_calc_fv": report['sip_calc_fv'], "future_target_amount": report['future_target_amount'], "future_target_years": report['future_target_years'], "future_req_monthly": report['future_req_monthly']}
